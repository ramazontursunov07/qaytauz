...
class ImageKitStorage(Storage):
    ...
    def _save(self, name, content):
        ...
        # MUHIM: imagekitio==2.2.8 kutubxonasining upload_file() metodi OBYEKT
        # emas, oddiy dict qaytaradi: {"error": ..., "response": ...}.
        # Muvaffaqiyatli bo'lsa "error" None bo'ladi va "response" ichida
        # yuklangan faylning ma'lumotlari (jumladan "url") bo'ladi.
        error = result.get("error") if isinstance(result, dict) else "kutilmagan javob turi"
        response_data = result.get("response") if isinstance(result, dict) else None

        if error or not response_data or not response_data.get("url"):
            raise IOError(
                "ImageKit'ga rasm yuklash muvaffaqiyatsiz bo'ldi. "
                f"Xatolik: {error}. Javob: {response_data}. "
